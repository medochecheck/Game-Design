...
enum PreprocessStatus preprocess(char* text_buf) {
    if (!text_buf) {
        return err_null;  // Если указатель NULL, возвращаем 0
    }
...
void fill_string_array(char* text_buf, size_t str_count, char** text) {
    if (!text_buf || !text) {
        ...
        return err_null; // Возвращаем код ошибки
    }
...
enum ExitStatus fill_string_array(char* text_buf, size_t str_count, char** text) {
    if (!text_buf || !text) {
        fprintf(stderr, "Error: NULL pointer passed to fill_string_array.\n");
        return err_memory; // Возвращаем код ошибки
    }

    size_t j = 0; // Индекс текущей строки
    ...
    for (size_t i = 0; text_buf[i] != '\0'; ++i) {
        if (text_buf[i] == '\n') {
            ...
            if (j < str_count) {
                ...
                return err_memory; // Прерываем выполнение при переполнении
            }
        }
    }

    // Обнуляем оставшиеся указатели, если их меньше чем str_count
    for (size_t k = j + 1; k < str_count; ++k) {
        text[k] = NULL;
    }

    return exit_ok; // Возвращаем успешный статус
}
...
//        if (text[i]) {  // Проверка на NULL указатель для каждой строки
//            printf("%s\n", text[i]);
//        } else {
//            printf("(null)\n");
//        }
